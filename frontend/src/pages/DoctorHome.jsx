...
import HorarioMedico from '../pages/HorarioMedico'; // Asumiendo que tienes un componente para crear horarios
...
export function DoctorHome({ onLogout }) {
    ...
    const [content, setContent] = useState('appointments'); // Estado para controlar el contenido
    ...
    return (
        <div className="min-h-full">
            <nav className="bg-gray-800">
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="hidden md:block">
                                <div className="ml-10 flex items-baseline space-x-4">
                                    <button onClick={() => setContent('appointments')} className="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white" aria-current="page">Citas</button>
                                    <button onClick={() => setContent('schedule')} className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Horarios</button>
                                    <button onClick={() => setContent('newSchedule')} className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Nuevo horario</button>
                                </div>
                            </div>
                        </div>
                        <div className="hidden md:block">
                ...
                <div className={`md:hidden ${isOpen ? 'block' : 'hidden'}`} id="mobile-menu">
                    <div className="space-y-1 px-2 pb-3 pt-2 sm:px-3">
                        <button onClick={() => setContent('appointments')} className="block rounded-md bg-gray-900 px-3 py-2 text-base font-medium text-white" aria-current="page">Citas</button>
                        <button onClick={() => setContent('schedule')} className="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Horarios</button>
                        <button onClick={() => setContent('newSchedule')} className="block rounded-md px-3 py-2 text-base font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Nuevo horario</button>
                    </div>
            ...
            <div className="container mx-auto p-4">
                {content === 'appointments' && <AppointmentsTable userId={userData.id} />}
                {content === 'schedule' && <div>Horarios</div>} {}
                {content === 'newSchedule' && <HorarioMedico />} {}
            </div>
        </div>
    );
}
